...
// Use existing isGitHubPages and basePath if already defined
if (typeof window.isGitHubPages === 'undefined') {
    window.isGitHubPages = window.location.hostname.includes('github.io') || (window.GITHUB_PAGES === true);
    console.log('Environment:', window.isGitHubPages ? 'GitHub Pages' : 'Local');
}

if (typeof window.basePath === 'undefined') {
    window.basePath = window.isGitHubPages ? '/FinBoard' : '';
    console.log('Base Path:', window.basePath);
}

// Use existing getPath function if already defined
if (typeof window.getPath === 'undefined') {
    window.getPath = (path) => {
